...
def about():
    ...
    return render_template('/users/about.html',form=form,success=success)
...
def password():
    ...
    if form.validate_on_submit():
        if user.check_password(form.current_password.data):
            ...
            incorrect = True
            success = False
    return render_template('/users/password.html',form=form,success=success,incorrect=incorrect)
...
@users.route('/tts')
def tts():
    return render_template('/users/tts.html')
    

@users.route('/transcribe')
def transcribe():
    return render_template('/users/transcribe.html',error=False)


@users.route('/emotions')
def emotions():
    return render_template('/users/emotions.html',error=False)
...
def upload():
    ...
    if request.method == 'POST':
        ...
        print("Try Again!")
        return render_template('/users/transcribe.html',error=True)
...
def uploademo(): 
    ...
    if request.method == 'POST':
        ...
        print("Try Again!")
        return render_template('/users/emotions.html',error=True)   
    ...
    if request.method == 'POST':
        ...
        if(user.membership=="Individual"):
            ...
            if(total_words<=user.words_left):
                ...
                return render_template('/users/tts.html', input_text=text1)
            else:
                return redirect(url_for('users.subscribe'))
        elif(user.membership=="Institutional"):
...
def subscribe():
    try:
        ...
        return redirect(url_for('users.login'))
    return render_template('/users/subscribe.html')
...
def success():
    ...
    db.session.commit()
    return render_template('/users/success.html',user=user,session=session,time=time,amount=amount)
...
def activity():
    try:
        ...
        return redirect(url_for('users.login'))
    return render_template('/users/activity.html',acts=acts)
